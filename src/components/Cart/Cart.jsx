...
const Cart = ()=> {
    const fetchCheckout = async (pizzas, totalPrice, values)=> {
      const string = pizzas.map(item=> {
        return `${item.title}, ${item.type}, ${item.size}см, цена: ${item.price}(${item.count}шт)`
      })
      axios.post("https://api.telegram.org/bot5765644333:AAGHUMbTDHv_u4W2hZZhaJp3JPJL4FFnNj0/sendMessage",
      {
        chat_id: "-1001318536633",
        parse_mode: "html",
    ...
    const onSubmitHandler = values=> {
      fetchCheckout(items, totalPrice, values)
    }
          ...
          <div className="cart">
            ...
            <div className="cart__bottom">
              <div className="cart__bottom-details">
                <span> Всего пицц: <b>{amount} шт.</b> </span>
              ...
              <form className={styles.form} onSubmit={handleSubmit(onSubmitHandler)}> 
                ...
                <div>
                  <input {...register("name")} placeholder="Имя" type="text" required={true}/>
                      <input {...register("tel")} placeholder="Телефон 0ххххххххх" type="tel" pattern="0[0-9]{9}" required={true}/>
